...
> **Status 2026-09-25: 5.1 (build 6) implemented, not yet committed or submitted.** Kai relaxed the frozen-4.3 rule for 5.1: visible game changes are allowed if they keep the legacy art and fonts. Both apps: a shuffled deck (no repeats until the whole list is seen), missed words come back (Leitner boxes in `ProgressStore`, saved to Application Support/progress.json), a pause after a correct answer that shows what all four choices mean (on by default; "Show all four meanings" toggle on About), holding a word on that pause (or in Recent Words) opens the system dictionary, falling back to the app's own definition when no dictionary is installed — not on the cards before answering, where the answer card's lookup would give the question away, best streak shown on About, and VoiceOver no longer repeats an unchanged definition. MoreBlanks only: a "Review mistakes only" mode, a Recent Words list and a Progress section. Blanks' About now advertises those features instead of "more words" (both apps always shipped the same list). Content: `scripts/content_fixes.py` rewrote 34 WordNet entries that used a proper-noun sense (haggard, martial, fez…) or gave the answer away, and dropped 6, leaving 8,103 words. Tests: 30. Baseline tag `pre-v5.1` (local only). Played through in the simulator 2026-09-25 (drag and tap modes, pause on/off, persistence, MoreBlanks review/recent/reset, iPad); that run caught a simultaneous LongPressGesture that stopped every card drag, and a fourth meaning clipped out of view. Still open: on-device check of VoiceOver and haptics, new screenshots, and deleting Blanks' stale 3.5"–5.5" screenshot sets in ASC. Parked for 5.2: easy word list (`archive/Blanks-old/pyword-new/simple.plist`), pronunciation, example sentences, word-of-the-day widget.
